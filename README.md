...
- 保存和激活账号前自动请求 `/0.6/user/details`，验证 API 地址与 OAuth Token 是否可用
...
3. 点击 **Add**，填写名称和平台，然后点击 **Authorize now (fully automatic)**；在自动打开的网页中登录并授权，Token 会自动回填。没有为 JOSM 注册 OAuth 应用的自定义平台仍可手工填写 Token。
